...
// parent.
func setupBaseDevice(nativeDevs []netlink.Link, mode string, mtu int) (netlink.Link, netlink.Link, error) {
	switch mode {
	case "flannel":
	...
	case "ipvlan":
		ipvlan, err := setupIpvlan("cilium_host", nativeDevs[0])
		if err != nil {
			return nil, nil, err
		}
	...
	default:
		...
		linkHost, err := netlink.LinkByName("cilium_host")
		if err != nil {
			return nil, nil, err
		}
		linkNet, err := netlink.LinkByName("cilium_net")
		if err != nil {
			return nil, nil, err
		}
